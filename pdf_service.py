...
def generate_pdf_checklist(titulo_tramite: str, requisitos_lista: list[str]) -> str:
    ...
    pdf.cell(0, 10, f'Trámite: {titulo_tramite}', new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 10, f'Fecha de consulta: {datetime.now().strftime("%d/%m/%Y")}', new_x="LMARGIN", new_y="NEXT")
    pdf.ln(10)
    
    # Lista de requisitos
    pdf.set_font('helvetica', size=12)
    pdf.cell(0, 10, 'Requisitos a presentar (marca la casilla al reunirlos):', new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    ...
    for req in requisitos_lista:
        ...
        pdf.multi_cell(0, 10, text=req_clean)
        pdf.ln(2)
